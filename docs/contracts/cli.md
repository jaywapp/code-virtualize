...
- 직계 member의 상대 순서만 바뀌면 `member-order-changed` fingerprint 증거로 남긴다. 순서는 member가 속한 선언 짝(아래 짝짓기) 순서와 선언 안 위치로 정하므로, partial 선언 하나를 같은 type의 다른 선언이 없는 파일로 옮기는 것만으로는 순서 변경이 아니다. 같은 type의 다른 partial 선언이 이미 있는 파일로 옮기면 같은 path 안의 순번 짝짓기가 어긋나 `body_changed`와 `member-order-changed`를 과보고할 수 있다(알려진 한계, TASK-043).
