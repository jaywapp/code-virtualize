...
| 원자적 교체 | generation은 `.tmp-<guid>`에 쓴 뒤 `Directory.Move`로 확정한다. pointer `current.json`은 `.current-<guid>.tmp` 다음 같은 볼륨 overwrite rename(`File.Move(..., overwrite: true)`)으로 교체하고, 외부 프로세스의 일시 잠금(오류 5·32·1175)은 상한 있는 재시도로 넘긴다. publish 뒤에도 이전 generation은 지우지 않는다. commit 뒤 pointer 교체 전에 crash가 나면 게시되지 않은 generation이 남는다(테스트로 확인). | `GenerationStore.Publish`, `GenerationStoreTests` |
